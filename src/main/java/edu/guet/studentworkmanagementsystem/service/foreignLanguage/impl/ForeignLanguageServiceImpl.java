...
public class ForeignLanguageServiceImpl extends ServiceImpl<ForeignLanguageMapper, ForeignLanguage> implements ForeignLanguageService {
    ...
    @Override
    public BaseResponse<List<ForeignLanguageItem>> getForeignLanguages(ForeignLanguageQuery query) {
        CompletableFuture<List<ForeignLanguageItem>> future = CompletableFuture.supplyAsync(() -> QueryChain.of(ForeignLanguage.class)
                .select(
                ...
                .listAs(ForeignLanguageItem.class), readThreadPool);
        List<ForeignLanguageItem> execute = FutureExceptionExecute.fromFuture(future).execute();
